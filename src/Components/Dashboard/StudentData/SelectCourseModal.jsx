...
function SelectCourseModal({ show, setShow }) {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        ...
        // 🚨 If already has preferredCourses, close modal
        if (found && found.preferredCourses && found.preferredCourses.length > 0) {
          setShow(false);
        }
      } catch (e) {
        console.error("Error fetching student:", e);
  ...
  // ✅ Prevent rendering modal if student already selected courses
  if (!show || !currentStudent || (currentStudent.preferredCourses && currentStudent.preferredCourses.length > 0)) {
    return null;
  }

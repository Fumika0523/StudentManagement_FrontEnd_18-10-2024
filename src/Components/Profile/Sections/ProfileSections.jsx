      ...
      >
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            ...
            >
              {field?.icon && (
                <Box sx={{ color: "#fff", display: "flex", "& .MuiSvgIcon-root": { fontSize: 17 } }}>
                  {field.icon}
                </Box>
              )}
            </Box>
            ...
            {!field?.disabled && (
              <IconButton
                ...
                sx={{
                  ...
                  border: "1px solid",
                  borderColor: isEditing ? "#bfdbfe" : "#e2e8f0",
                  "&:hover": {
                    background: "#eff6ff",
                    color: "#2563eb",
                    borderColor: "#bfdbfe",
...
export default function ProfileSections({
  ...
  // choose one object first
  const userData = singleStudentData || singleUserData || {};

  // if student profile does not include some account fields,
  // fallback to singleUserData where needed
  const role = safeText(singleUserData?.role || singleStudentData?.role);
  const isActive = Boolean(singleUserData?.isActive ?? singleStudentData?.isActive);
  const isStudent = role === "student";

  const initials = makeInitials(userData);

  const email = safeText(userData?.email || singleUserData?.email);
  const firstName = safeText(userData?.firstName);
  const lastName = safeText(userData?.lastName);
  const Id = safeText(userData?._id)
  const gender = safeText(userData?.gender);
  const country = safeText(userData?.country);
  const phoneNumber = safeText(userData?.phoneNumber || singleUserData?.phoneNumber);
  const birthdate = safeText(userData?.birthdate || userData?.birthday);

  const basicFields = [
    { key: "firstName", label: "First Name", icon: <PersonIcon  />, value: firstName.charAt(0).toUpperCase() + firstName.slice(1), disabled: false },
    { key: "lastName", label: "Last Name", icon: <PersonIcon />, value: lastName.charAt(0).toUpperCase() + lastName.slice(1), disabled: false },
    { key: "Account ID", label: "ID", icon: <MdBadge />, value: Id, disabled: true },
    { key: "gender", label: "Gender", icon: <WcIcon />, value: gender.charAt(0).toUpperCase() + gender.slice(1), disabled: false },
    { key: "country", label: "Country", icon: <PublicOutlined />, value: country.charAt(0).toUpperCase() + country.slice(1), disabled: false },
    ...(isStudent
      ? [
          {
            key: "birthday",
  ...
  const contactFields = [
    ...
    { key: "phoneNumber", label: "Phone Number", icon: <PhoneIcon />, value: phoneNumber, disabled: false },
  ];
        ...
        >
          <Box sx={{ display: "flex", alignItems: "flex-start", gap: 2.5, flexWrap: "wrap" }}>
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography sx={{ fontWeight: 800, fontSize: { xs: 16, sm: 18 }, color: "#0f172a", letterSpacing: "-0.02em", lineHeight: 1.25, mb: 0.3 }}>
                {loading ? "Loading…" :  `${firstName.charAt(0).toUpperCase() + firstName.slice(1)} ${lastName.charAt(0).toUpperCase() + lastName.slice(1)}`}
              </Typography>
              <Typography sx={{ fontSize: 13, color: "#94a3b8", mb: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {email}
              </Typography>
              <Stack direction="row" spacing={0.8} flexWrap="wrap" useFlexGap>
                <Chip size="small" label={isActive ? "Active" : "Inactive"} sx={{ fontSize: 10.5, fontWeight: 700, height: 20, borderRadius: "5px", bgcolor: isActive ? "#f0fdf4" : "#fef2f2", color: isActive ? "#16a34a" : "#dc2626", border: isActive ? "1px solid #bbf7d0" : "1px solid #fecaca" }} />
                <Chip size="small" label={role} sx={{ fontSize: 10.5, fontWeight: 700, height: 20, borderRadius: "5px", bgcolor: "#eff6ff", color: "#1a3bbd", border: "1px solid #bfdbfe" }} />
              </Stack>
            </Box>
          </Box>
        </Box>

...
function ModalAddCourse({ show, setShow, setCourseData }) {
  ...
  const addCourse = async (newCourse) => {
    try {
      const addRes = await axios.post(`${url}/addcourse`, newCourse, getAuthConfig());
      console.log("addRes",addRes.data.courseDetail.courseName)

      if (addRes.data) {
        const freshData = await axios.get(`${url}/allcourse`, getAuthConfig());
        setCourseData(freshData.data.courseData);
        toast.success(`${addRes.data.courseDetail.courseName} is added successfully!`);
